...
import { createOverlayViewClass } from '../OverlayView/OverlayViewClass'
 ...
 * @returns
 */
export const MarkerOverlay: FC<PropsWithChildren<MarkerOverlayProps>> = (props) => {
  const {
  ...
  }, [map, maps, pane, position, origin, originOffset, draggable, onDrag, onDragStart, onDragEnd])
...
function MarkerOverlayFactory(maps: typeof google.maps = google.maps) {
  return class OverlayView extends createOverlayViewClass(maps) {
    ...
    onMouseDown = (e: MouseEvent) => {
      ...
      this._div()?.addEventListener('mousemove', (e: MouseEvent) => {
        ...
        const currPxPos = this.getProjection()?.fromLatLngToDivPixel(this.position)
        if (!currPxPos) {
          return
        }
